...
class HistoryHandler(osmium.SimpleHandler):
    ...
    def add_element(self, element, type):
        self.count += 1
        if self.count % 10**6 == 0:
            print(f'Type: {type}, ID: {self.prev_element_id}, Count: {self.count}')
        if self.element_history and self.prev_element_id != element.id:
            processed_history = hhf.process_element_history(self.element_history, self.get_pre_data(self.prev_type)[self.prev_element_id], include_raw_tags=True)
            for element_dict in processed_history.values():
                if len(element_dict) == 1:
                    ...
                    copy_dict = deepcopy(element_dict)
                    for key in keys:
                        if key not in ['cs_id', 'id', 'type', 'tags', 'prev_tags']:
                            del element_dict[key]
                    if len(element_dict) != 5:
                        print(element_dict)
                        print(copy_dict)
...
if reverted_data:
    input_file = '/Users/dansvenonius/Desktop/Misc output/Reverted Data/relevant_reverted_elements.osm.pbf'
    ...
    input_file = '/Users/dansvenonius/Desktop/Misc output/Not Reverted Data/relevant_reverted_elements.osm.pbf'
...
processed_elements_df.to_csv(output_path + 'edited_elements_with_tags.csv', index=False)
